...
export function resolveHermesAgentDir(
  ...
  for (const candidate of candidates) {
    if (existsSync(resolve(candidate, 'hermes_cli'))) return candidate
  }
...
export function resolveHermesPython(agentDir: string): string {
  ...
  throw new Error(
    `No Hermes Python interpreter found. Expected one of: ${venvPython}, ${uvVenv}, ${nousPython}`,
  )
}

/**
 * Build the PATH environment variable for Hermes child processes.
 * Ensures hermes binary, local bin, and agent virtualenv are on PATH.
 */
export function buildHermesPath(agentDir?: string | null): string {
  return [
